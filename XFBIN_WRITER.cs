...
namespace XFBIN_LIB
{
    public class XFBIN_WRITER
    {
        public static XFBIN.XFBIN ReadDirectoryXFBIN(string path)
        {
            ...
            // Read page JSON files using the correct encoding (e.g., windows-1251)
            foreach (string file in Directory.EnumerateFiles(path, "*.json", SearchOption.AllDirectories))
            {
                ...
                // Read the JSON text using Windows-1251 encoding
                string jsonText = File.ReadAllText(file, Encoding.GetEncoding("windows-1251"));
                // Deserialize directly from the string
                READ_PAGE page = JsonSerializer.Deserialize<READ_PAGE>(jsonText, options);
                page_list.Add(page);
            }

            // Merge string values to avoid duplicates
            ...
            // Fill lists from merged strings
            ...
            // Create global chunk map list from JSONs
            ...
            List<READ_CHUNK_MAP> merged_chunk_map_list = pages_chunk_map_list
                .GroupBy(car => (car.TypeName, car.ChunkName, car.FilePathName))
                .Select(g => g.First())
                .ToList();
            ...
            foreach (READ_PAGE page in page_list)
            {
                foreach (READ_EXTRA_CHUNK_MAP_INDICES _ch_map in page.ExtraMappings)
                {
                    ...
                    int chunk_map_index = merged_chunk_map_list.FindIndex(r =>
                        (r.ChunkName == _ch_map.ChunkMapName &&
                         r.TypeName == _ch_map.ChunkTypeName &&
                         r.FilePathName == _ch_map.ChunkPathName));
                ...
                foreach (READ_CHUNK_MAP _ch_map in page.ChunkMappings)
                {
                    ...
                    int chunk_map_index = merged_chunk_map_list.FindIndex(r =>
                        (r.ChunkName == _ch_map.ChunkName &&
                         r.TypeName == _ch_map.TypeName &&
                         r.FilePathName == _ch_map.FilePathName));
                    new_chunk_map_index.ChunkMapIndex = (uint)chunk_map_index;
                    chunk_map_indices.Add(new_chunk_map_index);
                }
            }

            // Create chunk table using UTF8 byte counts
            ...
            foreach (READ_PAGE page in page_list)
            {
                ...
                for (int i = page_offset; i < page.ChunkMappings.Count() + page_offset; i++)
                {
                    READ_CHUNK_MAP temp_chunk_map = merged_chunk_map_list[(int)chunk_map_indices[i].ChunkMapIndex];
                    CHUNK_MAP temp2_chunk_map = chunk_table.ChunkMaps[(int)chunk_map_indices[i].ChunkMapIndex];
                ...
                for (int i = extramap_offset; i < page.ExtraMappings.Count() + extramap_offset; i++)
                {
                    EXTRA_CHUNK_MAP_INDICES temp_extra_map = chunk_table.ExtraMappings[i];
                ...
                null_chunk.ChunkMapIndex = (uint)temp_chunk_map_for_page.FindIndex(r =>
                    (r.ChunkName == "" && r.TypeName == "nuccChunkNull" && r.FilePathName == ""));
                ...
                foreach (READ_CHUNK chunk in page.Chunks)
                {
                    ...
                    new_chunk.ChunkData = File.ReadAllBytes(Path.Combine(page_path, chunk.file_name));
                    new_chunk.Size = (uint)new_chunk.ChunkData.Length;
                    new_chunk.ChunkMapIndex = (uint)temp_chunk_map_for_page.FindIndex(r =>
                        (r.ChunkName == chunk.ChunkMapName &&
                         r.TypeName == chunk.ChunkTypeName &&
                         r.FilePathName == chunk.ChunkPathName));
                ...
                page_chunk.ChunkMapIndex = (uint)temp_chunk_map_for_page.FindIndex(r =>
                    (r.ChunkName == "Page0" && r.TypeName == "nuccChunkPage" && r.FilePathName == ""));
            ...
            xfbin_file.ChunkTableSize = 0x28 + chunk_table.ChunkNameSize + chunk_table.ChunkTypeSize +
                                          chunk_table.FilePathSize + chunk_table.ChunkMapSize +
                                          (chunk_table.ChunkMapIndicesCount * 4);
        ...
        public static void RepackXFBIN(string path)
        {
            ...
            using (EndianBinaryWriter writer = new EndianBinaryWriter(EndianBitConverter.Big, File.Open(outputFile, FileMode.CreateNew)))
            {
                ...
                // Write type list using Windows-1251 encoding
                foreach (CHUNK_TYPE type in xfbin_file.ChunkTable.ChunkTypes)
                {
                    writer.Write(Encoding.GetEncoding("windows-1251").GetBytes(type.ChunkTypeName));
                    writer.Write(new byte[1]);
                }
                // Write path list using Windows-1251 encoding
                foreach (FILE_PATH file_path in xfbin_file.ChunkTable.FilePaths)
                {
                    writer.Write(Encoding.GetEncoding("windows-1251").GetBytes(file_path.FilePathName));
                    writer.Write(new byte[1]);
                }
                // Write name list using Windows-1251 encoding
                foreach (CHUNK_NAME chunk_name in xfbin_file.ChunkTable.ChunkNames)
                {
                    writer.Write(Encoding.GetEncoding("windows-1251").GetBytes(chunk_name.ChunkName));
                    writer.Write(new byte[1]);
                }
